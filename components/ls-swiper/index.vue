<template>
	<view class="wrap">
		<swiper class="swiper" :style="{height: height *2 + 'rpx',  width: width *2 + 'rpx' }" :autoplay="autoplay" :interval="interval" :duration="duration"
		 :circular='loop' @change='change' :previous-margin='previousMargin + "rpx"' :next-margin='nextMargin + "rpx"'>
			<swiper-item v-for="(item,index) in list" :key='index' @click="$emit('clickItem',item)">
				...
				<view  style="position: absolute;">
					<view style="position: absolute;top: -110rpx;font-size: 14px;color: #FFFFFF;width: 84px;height: 20px;left: 40rpx;">最新欧洲泌尿</view>
					<view style="position: absolute;top: -70rpx;font-size: 18px;color: #FFFFFF;width: 72px;height: 25px;left: 40rpx;">随访指南</view>
				</view>
			</swiper-item>
		</swiper>
